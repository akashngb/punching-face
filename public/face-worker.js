  ...
  try {
    ...
    if (data.type === 'frame') {
      try {
        ...
        if (quality.ok) {
          const xs = quality.oval.map((p) => p.x * w),
            ys = quality.oval.map((p) => p.y * h);
          const x0 = Math.min(...xs),
            x1 = Math.max(...xs),
            y0 = Math.min(...ys),
            y1 = Math.max(...ys);
          if (Math.abs(quality.yaw) < 35 || !headBox)
            headBox = {
              cx: (x0 + x1) / 2,
              top: y0 - (y1 - y0) * 0.85,
              bottom: y1 + (y1 - y0) * 0.12,
              width: Math.max((x1 - x0) * 1.65, (y1 - y0) * 1.15),
              height: y1 - y0,
            };
        }
        ...
        if (!headBox) {
          self.postMessage({
            ...
            ok: false,
            message:
              'Start facing the camera, with the entire head visible, to locate the head.',
          });
          return;
        }
        ...
        try {
          ...
          for (let y = 0; y < mh; y++)
            for (let x = 0; x < mw; x++)
              if (
                labels[y * mw + x] === 1 &&
                (y * h) / mh > headBox.top &&
                (y * h) / mh < headBox.bottom &&
                Math.abs((x * w) / mw - headBox.cx) < headBox.width * 0.75
              ) {
                sumX += (x * w) / mw;
                hairCount++;
              }
          const cx =
            hairCount > 20 ? headBox.cx * 0.4 + (sumX / hairCount) * 0.6 : headBox.cx;
          const polygon = new OffscreenCanvas(w, h),
            pc = polygon.getContext('2d', { willReadFrequently: true });
          if (quality.ok) {
            pc.fillStyle = 'white';
            pc.beginPath();
            quality.oval.forEach((p, i) =>
              i ? pc.lineTo(p.x * w, p.y * h) : pc.moveTo(p.x * w, p.y * h),
            );
            pc.closePath();
            pc.fill();
          }
          ...
          for (let y = 0; y < h; y++)
            for (let x = 0; x < w; x++) {
              ...
              const inHead =
                Math.abs(x - cx) < headBox.width * 0.5 &&
                y > headBox.top &&
                y < headBox.bottom;
              const keep =
                faceAlpha[i + 3] > 200 ||
                (inHead &&
                  (label === 1 ||
                    label === 3 ||
                    (label === 2 && y < headBox.bottom - headBox.height * 0.04)));
              if (keep) {
                pixels.data[i + 3] = 255;
                count++;
