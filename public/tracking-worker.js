...
self.onmessage=async({data})=>{
  try{
    if(data.type==='init'){
      ...
      try{detector=await HandLandmarker.createFromOptions(files,{baseOptions:{modelAssetPath:`${data.origin}/models/hand_landmarker.task`,delegate:'GPU'},runningMode:'VIDEO',numHands:2,minHandDetectionConfidence:.5,minTrackingConfidence:.4});}
      catch{detector=await HandLandmarker.createFromOptions(files,{baseOptions:{modelAssetPath:`${data.origin}/models/hand_landmarker.task`,delegate:'CPU'},runningMode:'VIDEO',numHands:2,minHandDetectionConfidence:.5,minTrackingConfidence:.4});}
      self.postMessage({type:'ready'});
    }else if(data.type==='enablePose'){
      if(poseDetector){self.postMessage({type:'poseReady'});return;}
      ...
      try{poseDetector=await PoseLandmarker.createFromOptions(files,{baseOptions:{modelAssetPath:`${origin}/models/pose_landmarker.task`,delegate:'GPU'},runningMode:'VIDEO',numPoses:1,outputSegmentationMasks:wantSegmentation});}
      catch{poseDetector=await PoseLandmarker.createFromOptions(files,{baseOptions:{modelAssetPath:`${origin}/models/pose_landmarker.task`,delegate:'CPU'},runningMode:'VIDEO',numPoses:1,outputSegmentationMasks:wantSegmentation});}
