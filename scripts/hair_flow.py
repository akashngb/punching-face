...
def orientation_field(rgb):
    ...
    xx, yy, xy = [
        sum(
            cv2.GaussianBlur(v, (0, 0), s) * w
            for s, w in [(1.2, 0.5), (2.5, 0.35), (5, 0.15)]
        )
        for v in (gx * gx, gy * gy, gx * gy)
    ]
    theta = 0.5 * np.arctan2(2 * xy, xx - yy) + np.pi / 2
    ...
    confidence *= np.clip(np.sqrt(xx + yy) / 0.12, 0, 1)
    ...
    for path in paths:
        ...
        for a, b in zip(path[:-1], path[1:]):
            if np.linalg.norm(b - a) < 1:
                continue
            samples = a + np.linspace(0, 1, max(2, int(np.linalg.norm(b - a) / 3)))[
                :, None
            ] * (b - a)
            points.extend(samples)
            directions.extend(np.tile(unit(b - a), (len(samples), 1)))
...
def lift_direction(cam, pose, xy, direction, normal, B):
    ...
    dr = (
        np.c_[
            cam.cam_from_img(xy + direction) - cam.cam_from_img(xy), np.zeros(len(xy))
        ]
...
def photo_field(
    ...
    observed = np.zeros(n)
    votes = np.zeros(n)
    foreground = np.zeros(n)
    directed = np.zeros((n, 3))
    direction_weight = np.zeros(n)
    audit = []
    images = {im.name: im for im in rec.images.values()}
    from scripts.photo_detail import detail_image

    color_best = np.zeros(n)
    best_colors = np.tile(spec['hair']['colorSrgb'], (n, 1))
    for view in spec.get('views', []):
        ...
        h, w = px.shape[:2]
        crop = np.array(spec['crops'][im.name])
        ...
        facing = np.maximum(
            np.sum((normals @ B) * unit(im.projection_center() - world), axis=1), 0
        )
        visibility = inside * facing**3 * (facing > 0.18)
        if surface is not None:
            from scripts.photo_geometry import zbuffer

            vertices, faces = surface
            vc = (
                vertices / transform['scale'] @ B + center
            ) @ pose.rotation.matrix().T + pose.translation
            depth = zbuffer(cam.img_from_cam(vc), vc[:, 2], faces, w, h)
            visibility *= np.abs(cp[:, 2] - depth[y, x]) * transform['scale'] < 0.007
        observed += visibility
        foreground += visibility * (px[y, x, 3] > 220)
        hit = mask[y, x]
        votes += visibility * hit
        detailed = detail_image(folder, im.name)
        factor = detailed.shape[1] / w
        ...
        take = color_quality > color_best
        best_colors[take] = detailed[dy[take], dx[take], :3] / 255.0
        color_best = np.maximum(color_best, color_quality)
        if colors_only:
            continue
        flow, confidence = orientation_field(detailed[:, :, :3])
        direction = flow[dy, dx].copy()
        confidence = confidence[dy, dx]
        tree, paths = path_samples(view.get('hairFlowPaths', []), crop)
        semantic_weight = np.zeros(n)
        if tree is not None:
            distance, nearest = tree.query(
                np.nan_to_num(xy, nan=-1e5, posinf=-1e5, neginf=-1e5)
            )
            semantic = paths[nearest]
            direction *= np.where(np.sum(direction * semantic, axis=1) < 0, -1, 1)[
                :, None
            ]
            semantic_weight = np.exp(
                -((distance / max(12, (crop[2] - crop[0]) * 0.09)) ** 2)
            )
            blend = semantic_weight * (1 - confidence * 0.75)
            direction = unit(
                direction * (1 - blend[:, None]) + semantic * blend[:, None]
            )
        weight = visibility * hit * np.maximum(confidence, semantic_weight * 0.45)
        valid = weight > 0.002
        d3 = np.zeros((n, 3))
        if valid.any():
            d3[valid] = lift_direction(
                cam, pose, xy[valid], direction[valid], normals[valid], B
            )
        ...
        audit.append(
            {
                'filename': im.name,
                'supportedSamples': int(valid.sum()),
    ...
    return {
        ...
        'colors': colors,
        'confidence': np.clip(support, 0, 1) * coherence,
        'coverage': coverage,
        'observed': observed,
        'foreground': foreground,
        'directed': direction_weight > 0.02,
        'views': audit,
    }


def trace_locks(
    roots, normals, directions, confidence, parameters, hairline, segments=18
):
    ...
    top = np.clip((roots[:, 1] - hairline + 0.025) / 0.055, 0, 1)
    posterior = np.clip((-normals[:, 2] - 0.15) / 0.65, 0, 1) * (
        1 - np.clip(normals[:, 1], 0, 1)
    )
    top *= 1 - posterior
    full_length = (
        parameters['sideLengthMm'] * (1 - top) + parameters['lengthMm'] * top
    ) * 0.001
    # Exterior visible sections of longer strands, not invented scalp roots.
    fraction = parameters.get('visibleSectionFraction', 0.55)
    lengths = np.clip(full_length * fraction, 0.004, 0.055)
    step = lengths / segments
    mid = segments // 2
    surface_normals[:, mid] = normals
    for sign in (-1, 1):
        q = roots.copy()
        previous = directions * sign
        for j in range(1, mid + 1):
            ...
            if distances.ndim == 1:
                ...
                near = near[:, None]
            weights = np.exp(
                -((distances / np.maximum(distances[:, -1:], 0.002)) ** 2) * 3
            )
            ...
            valid = distances[:, 0] < 0.0035
            q = np.where(valid[:, None], projected, q)
            local = directions[near].copy()
            local *= np.where(np.sum(local * previous[:, None], axis=2) < 0, -1, 1)[
                :, :, None
            ]
            flow = unit(np.sum(local * weights[:, :, None], axis=1))
            previous = unit(previous * 0.65 + flow * 0.35)
...
def bind_curve_points(vertices, faces, points):
    ...
    triangle = near[np.arange(len(points)), choice]
    closest = closest.reshape(len(points), near.shape[1], 3)[
        np.arange(len(points)), choice
    ]
    ...
    return {
        'triangles': faces[triangle].ravel().tolist(),
        'weights': np.round(bary, 7).ravel().tolist(),
    }


def conform_curve_points(vertices, faces, points, vertex_normals):
    """Project traced stations onto the mesh used for appearance and rigging.

    Moving least squares traces flow smoothly but is not the triangle surface:
    even a fraction of a millimetre of penetration can hide an entire fiber.
    Reuse the very same bindings for rest geometry and subsequent deformation.
    """
    bindings = bind_curve_points(vertices, faces, points)
    triangles = np.asarray(bindings['triangles']).reshape(-1, 3)
    weights = np.asarray(bindings['weights']).reshape(-1, 3)
    weights /= weights.sum(1, keepdims=True)
    bindings['weights'] = weights.ravel().tolist()
    surface = np.einsum('ij,ijk->ik', weights, vertices[triangles])
    normals = unit(np.einsum('ij,ijk->ik', weights, vertex_normals[triangles]))
    return surface, normals, bindings
