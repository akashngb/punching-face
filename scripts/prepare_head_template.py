...
def load():
    ...
    for line in (ROOT / 'public/head-template/base.obj').read_text().splitlines():
        ...
        if not t:
            continue
        if t[0] == 'v':
            p.append(list(map(float, t[1:4])))
        elif t[0] == 'g':
            group = t[1]
        elif t[0] == 'f' and group == 'body':
            quads.append([int(x.split('/')[0]) - 1 for x in t[1:]])
    p = np.array(p)
    quads = np.array(quads)
    quads = quads[np.min(p[quads, 1], axis=1) > 5.8]
    ids = np.unique(quads)
...
if __name__ == '__main__':
    p, f, q, ids = load()
    d = {
        'positions': p.ravel().tolist(),
        'indices': f.ravel().tolist(),
        'sourceIds': ids.tolist(),
        'quads': q.tolist(),
    }
    (ROOT / 'public/head-template/head.json').write_text(json.dumps(d))
    print(len(p), len(f), p.min(0), p.max(0))
