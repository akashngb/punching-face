...
SUMMARIZE = (
    ROOT
    / '.local/third_party/yibuapi-examples/yibuapi_examples_20260918_v01/summarize_usage.py'
)
...
def main() -> int:
    if not SUMMARIZE.is_file():
        print(
            f'Sponsor package not extracted at {SUMMARIZE.parent}. Fetch and extract '
    ...
    if not LEDGER.is_file():
        print(
            f'No ledger at {LEDGER}. Run the app and trigger at least one OMNI call first.',
            file=sys.stderr,
        )
        return 3
    ...
    print()
    print(
        'Before sending: `source.path` in the JSON is an absolute local path — sanitize if needed.'
    )
