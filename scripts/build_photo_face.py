...
from scripts.photo_geometry import camera_rays,robust_landmarks,projection_error,make_surface,bake_photographs,eye_parts
...
def run(folder,use_astra=True):
    evidence={'source':'photographs','usesSplats':False};started=time.perf_counter();timer=PipelineTimer(folder)
    def status(stage,message):
        timer.mark(stage)
        atomic(folder/'status.json',{'status':'running','stage':stage,'message':message,'evidence':evidence})
    try:
        status('photos','Building your 3D face from saved photographs…');rec,info=recover(folder,status);evidence.update(info)
        status('geometry','Triangulating facial measurements and checking reserved views…')
        ...
        if use_astra:
            ...
            if head_capture and advice['hair']['present']:advice=hair_completion(advice,recognize_hair(folder,advice))
        ...
        status('eyes','Scanning the sharpest recorded eyes; generating missing detail with Astra when needed…' if use_astra else 'Checking recorded eyes and preparing labeled local eye estimates…')
        eyes=scan_eyes(folder,manifest['frames'],use_astra)
        evidence['eyeDetail']={k:eyes[k] for k in ['summary','model','geometryEstimated','generationMethod','apiError']}
        evidence['eyeDetail']['eyes']={name:{k:spec[k] for k in ['mode','qualityReason','candidate']} for name,spec in eyes['eyes'].items()}
        ...
        if groom:evidence['hairGroom'].update(version=groom['version'],recognition=groom.get('recognition'),**groom.get('evidence',{}))
        status('rig','Binding the facial controls and tissue cage…');anchors=physics_binding(folder,p,f,face_count)
        ...
        status('texture','Applying the photographs and fitted eye detail to the 3D mesh…');stats['appearance']=bake_photographs(folder,p,f,face_count,rec,frames,texture_views,center,B,transform,advice,eyes);stats['seconds']=round(time.perf_counter()-started,2);atomic(folder/'mesh.json',data)
        timer.finish()
        ...
        timer.finish('failed');atomic(folder/'status.json',{'status':'failed','stage':'failed','message':str(e),'evidence':evidence});raise

if __name__=='__main__':
    parser=argparse.ArgumentParser();parser.add_argument('folder',type=Path);parser.add_argument('--local-only',action='store_true');args=parser.parse_args()
    ...
    try:import sponsor_obs;sponsor_obs.init('pipeline');sponsor_obs.patch_pipeline_timer();trace=sponsor_obs.continue_from_env('build_photo_face')
    except ImportError:from contextlib import nullcontext;trace=nullcontext()
    with trace:run(args.folder.resolve(),not args.local_only)
