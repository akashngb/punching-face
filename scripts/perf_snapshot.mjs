...
async function version(){ try { return await (await fetch(`http://127.0.0.1:${port}/json/version`)).json(); } catch { return null; } }

async function ensureChrome(){
  if (await version()) return null;
  if (!CHROME) throw new Error('No Chrome found. Set CHROME_PATH, or start Chrome with --remote-debugging-port=' + port);
  ...
  const child = spawn(CHROME, [`--remote-debugging-port=${port}`, `--user-data-dir=${profile}`,
    '--no-first-run', '--no-default-browser-check', '--new-window', url], {stdio: 'ignore', detached: false});
  for (let i = 0; i < 60; i++) { if (await version()) return child; await sleep(500); }
...
async function pageTarget(){
  for (let i = 0; i < 40; i++) {
    ...
    const hit = list.find(t => t.type === 'page' && t.url.startsWith(url.split('?')[0]));
    if (hit) return hit;
    if (i === 0) await fetch(`http://127.0.0.1:${port}/json/new?${encodeURIComponent(url)}`, {method: 'PUT'}).catch(() => {});
    await sleep(500);
  }
...
try {
  ...
  if (visible !== 'visible') throw new Error(`Tab is "${visible}" — rAF is throttled and the sample would be meaningless. Un-minimise the Chrome window and retry.`);
  await evaluate(send, `(async()=>{for(let i=0;i<120;i++){if(window.__punchingFace?.state?.ready)return true;await new Promise(r=>setTimeout(r,250));}throw new Error('app never reported ready')})()`);
  ...
  await writeFile(perfPath, before.replace(marker, `${row}
${marker}`));
  console.log(row);
  console.log(`Appended to docs/perf.md. Physics metrics: ${JSON.stringify(busy.metrics)}`);
} finally {
  ws.close();
