...
for name, url in {
    ...
    fetch(url, Path('public/models') / name)
(ROOT / 'public/vendor').mkdir(parents=True, exist_ok=True)
shutil.copyfile(
    ROOT / 'node_modules/@mediapipe/tasks-vision/vision_bundle.cjs',
    ROOT / 'public/vendor/vision_bundle.cjs',
)
shutil.copytree(
...
for name in ('LeePerrySmith.glb', 'Map-COL.jpg'):
    fetch(
        'https://raw.githubusercontent.com/mrdoob/three.js/dev/examples/models/gltf/LeePerrySmith/'
        + name,
        Path('public/reference') / name,
    )
archive = fetch(
    'https://raw.githubusercontent.com/aigc3d/LAM_WebRender/main/asset/arkit/p2-1.zip',
    Path('.local/sources/lam/p2-1.zip'),
)
with zipfile.ZipFile(archive) as z:
    for name in ('skin.glb', 'offset.ply', 'animation.glb', 'vertex_order.json'):
        path = ROOT / '.local/sources/lam/p2-1' / name
        path.parent.mkdir(exist_ok=True)
        path.write_bytes(z.read('p2-1/' + name))
fetch(
    'https://raw.githubusercontent.com/aigc3d/LAM_WebRender/main/LICENSE',
    Path('public/online-face/LICENSE.txt'),
)
if platform.system() == 'Darwin' and platform.machine() == 'arm64':
    name = 'brush-app-aarch64-apple-darwin.tar.xz'
    archive = fetch(
        'https://github.com/ArthurBrussee/brush/releases/download/v0.3.0/' + name,
        Path('.local/tools/brush') / name,
    )
    expected = '65b2631398c839be3c1d4d7160fe2326389dec87830aac0710985e6690a1048c'
    if hashlib.sha256(archive.read_bytes()).hexdigest() != expected:
        raise ValueError('Brush archive checksum mismatch.')
    destination = ROOT / '.local/tools/brush'
    with tarfile.open(archive) as t:
        members = t.getmembers()
        for m in members:
            if (
                not (destination / m.name)
                .resolve()
...
print(
    'Public assets ready. Run make_fixture.py, prepare_lam.py, and bake_texture.py next.'
)
