...
mesh = trimesh.load(
    root / "public/reference/LeePerrySmith.glb", force="mesh", process=False
)
np.random.seed(17)
points, ids = trimesh.sample.sample_surface(mesh, 100000)
# Remove the shoulder base so the target is a head at human scale.
floor = mesh.bounds[0, 1] + (mesh.bounds[1, 1] - mesh.bounds[0, 1]) * 0.25
keep = points[:, 1] > floor
...
axis = np.tile([0.0, 0.0, 1.0], (len(points), 1))
...
size = np.max(np.ptp(points, axis=0)) * 0.004
names = [
    "x",
    "y",
    "z",
...
values = np.column_stack(
    (
        points,
        normals,
        (colors - 0.5) / 0.28209479177387814,
        np.full(len(points), 4.0),
        np.tile(np.log([size, size, size * 0.07]), (len(points), 1)),
        quat[:, [3, 0, 1, 2]],
    )
)
...
result["stats"]["fixture"] = (
    'Synthetic surface Gaussians sampled from the CC BY 3.0 Lee Perry-Smith '
    'scan. Not a trained photo splat.'
)
