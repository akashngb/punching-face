...
def decode(data, extension="ply", min_opacity=.15):
    ...
    if extension == "splat":
        ...
        dt = np.dtype([("xyz", "<f4", 3), ("scale", "<f4", 3),
        ...
        color, opacity = v["rgba"][:, :3] / 255., v["rgba"][:, 3] / 255.
        normal = None
    else:
        ply = PlyData.read(io.BytesIO(data))
        if "vertex" not in ply:
            raise ValueError("PLY must have vertices; compressed PLY is not supported by the CPU converter.")
        v = ply["vertex"].data
        names = v.dtype.names
        if any(k.startswith("binding_") for k in names):
            raise ValueError("This avatar uses mesh-local Gaussian coordinates. Export it to world space with its matching FLAME surface before Poisson extraction; raw coordinates are not a face.")
        if not all(k in names for k in ("x", "y", "z")):
            raise ValueError("Export an uncompressed Gaussian PLY with x/y/z properties.")
        xyz = np.column_stack([v[k] for k in ("x", "y", "z")]).astype(float)
        scales = np.exp(np.clip(np.column_stack([v[f"scale_{k}"] for k in range(3)]), -20, 10)) if "scale_0" in names else None
        q = np.column_stack([v[f"rot_{k}"] for k in (1, 2, 3, 0)]).astype(float) if "rot_0" in names else None
        opacity = 1 / (1 + np.exp(-np.clip(v["opacity"], -30, 30))) if "opacity" in names else np.ones(len(xyz))
        if "f_dc_0" in names:
            color = np.clip(.5 + .28209479177387814 * np.column_stack([v[f"f_dc_{k}"] for k in range(3)]), 0, 1)
        elif "red" in names:
            color = np.column_stack([v[k] for k in ("red", "green", "blue")]) / 255.
        else:
            color = np.tile([.66, .43, .32], (len(xyz), 1))
        normal = np.column_stack([v[k] for k in ("nx", "ny", "nz")]).astype(float) if "nx" in names else None
        if normal is not None and np.mean(np.linalg.norm(normal, axis=1)) < .1:
            normal = None
    if len(xyz) < 100:
        raise ValueError("At least 100 surface samples are required.")
    if len(xyz) > 2_000_000:
        raise ValueError("Crop the face to fewer than two million splats before conversion.")
    valid = np.isfinite(xyz).all(axis=1) & np.isfinite(color).all(axis=1) & (opacity > min_opacity)
    if q is not None:
        valid &= np.isfinite(q).all(axis=1) & (np.linalg.norm(q, axis=1) > .01)
    if scales is not None:
        valid &= np.isfinite(scales).all(axis=1) & (scales > 0).all(axis=1)
    if normal is not None:
        valid &= np.isfinite(normal).all(axis=1) & (np.linalg.norm(normal,axis=1) > .01)
    if np.count_nonzero(valid) < 100:
        raise ValueError("Too few valid Gaussian samples remain after opacity filtering.")
...
def reconstruct(data, extension="ply", depth=7, max_triangles=26000, min_opacity=.15):
    ...
    lo, hi = np.quantile(xyz, [.005, .995], axis=0)
    extent = float(np.max(hi - lo))
    if extent < 1e-9:
        raise ValueError("The point cloud has no measurable extent.")
    center = (lo + hi) / 2
    scale = .28 / max(hi[1] - lo[1], extent * .3)
    crop = ((xyz >= lo - extent * .04) & (xyz <= hi + extent * .04)).all(axis=1)
    ...
    if normals is None:
        pc.estimate_normals(o3d.geometry.KDTreeSearchParamHybrid(radius=.025, max_nn=40))
    ...
    try:
        ...
        raise ValueError("Could not orient the surface normals. Use a denser cropped face scan.") from exc
    ...
    mesh, density = o3d.geometry.TriangleMesh.create_from_point_cloud_poisson(pc, depth=depth, scale=1.08, linear_fit=False, n_threads=4)
    mesh.remove_vertices_by_mask(np.asarray(density) < np.quantile(density, .025))
    ...
    if len(mesh.triangles) < 30:
        raise ValueError("No usable surface reconstructed. Check crop, normals, and splat alignment.")
    ...
    return {"positions":vertices.astype(np.float32).ravel().tolist(),
            ...
            "stats":{"sourceSplats":source_count, "surfaceSamples":len(pc.points), "vertices":len(vertices),
                     "triangles":len(mesh.triangles), "seconds":round(elapsed, 3), "depth":depth,
                     "medianThinness":alignment, "normalSource":normal_source, "minimumOpacity":min_opacity,
                     ...
                     "limitation":"Requires a cropped, surface-aligned face. Vertex colors retain DC appearance only. Not a guarantee of likeness or valid deformation topology."}}
