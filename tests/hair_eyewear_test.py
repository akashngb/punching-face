...
class HairEyewearTests(unittest.TestCase):
    ...
    def test_opaque_rim_cleanup_cannot_erase_landmark_brow_region(self):
        ...
        px=np.full((100,100,4),255,np.uint8);px[:,:,:3]=[175,120,85]
        landmarks=[{'x':.5,'y':.5} for _ in range(468)]
        for group in BROWS:
            for j,i in enumerate(group):landmarks[i]={'x':.2+.025*(j%5),'y':.28+.04*(j//5)}
        frame={'landmarks':landmarks};protected=brow_mask(px.shape,frame)>0
        px[protected,:3]=[35,25,15]
        clean,mask,_=clean_view(px,'front',self.spec(),frame,return_details=True,projected_mask=np.full((100,100),255,np.uint8))
        np.testing.assert_array_equal(clean[protected],px[protected]);self.assertFalse(mask[protected].any())
    ...
    def test_profile_contour_controls_temple_and_cannot_double_back_at_hinge(self):
        ...
        camera=SimpleNamespace(cam_from_img=lambda xy:np.c_[-xy[:,0],.5-xy[:,1]])
        im=SimpleNamespace(name='side',camera_id=1,projection_center=lambda:np.array([.5,0,0]),cam_from_world=lambda:SimpleNamespace(rotation=SimpleNamespace(matrix=lambda:rotation)))
        rec=SimpleNamespace(images={1:im},cameras={1:camera})
        spec={'views':[{'filename':'side','imageRightTemple':[[0,.38],[.1,.38],[.2,.38],[.3,.4]]}],'crops':{'side':[0,0,1,1]}}
        hinge=np.array([.08,.05,0]);p=np.array([[.08,.05,-.04]])
        q,audit=fit_temple(hinge,1,spec,rec,np.zeros(3),np.eye(3),1,p)
        self.assertEqual(audit['method'],'profile-contour');np.testing.assert_array_equal(q[0],hinge)
        self.assertTrue((np.diff(q[:,2])<0).all());self.assertLess(q[-1,1],q[0,1]);self.assertGreater(q[0,2]-q[-1,2],.1)
        q,audit=fit_temple(hinge,1,{'views':[]},rec,np.zeros(3),np.eye(3),1,p)
        self.assertEqual(audit['method'],'estimated-ear-hook');self.assertTrue(np.isfinite(q).all())
