...
joint = {'x': 0.5, 'y': 0.5, 'z': 0, 'visibility': 1}
frames = [
    {
        'image': image,
        'original': image,
...
request = Request(
    'http://127.0.0.1:5174/api/arm-capture',
    data=json.dumps(
        {
            'side': 'left',
            'frames': frames,
            'testFixture': 'blank images: MUST fail, not a personal scan',
        }
    ).encode(),
...
while time.monotonic() < deadline:
    state = json.load(
        urlopen('http://127.0.0.1:5174/api/arm-status?id=' + result['id'], timeout=10)
    )
    if state['status'] in ('failed', 'complete'):
        break
    time.sleep(0.5)
...
print(
    json.dumps(
        {
            'test': 'blank capture is rejected without a substitute mesh',
            'status': state,
        },
        indent=2,
    )
