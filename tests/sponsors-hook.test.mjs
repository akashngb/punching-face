...
test('index.html still loads the sponsor dock after main.js',()=>{
  ...
  assert.match(html,/src="\/src\/main\.js"><\/script><script type="module" src="\/src\/sponsors\/boot\.js">/);
});
...
test('the Python services keep their optional Sentry hooks, so one click still reads as one trace',()=>{
  ...
  const pipeline=read('face_pipeline.py');assert.match(pipeline,/from sponsor_obs import child_env as trace_env/);assert.match(pipeline,/subprocess\.Popen\([^\n]*env=trace_env\(\)\)/,'the pipeline subprocess no longer inherits the trace');
  const build=read('scripts/build_photo_face.py');assert.match(build,/sponsor_obs\.patch_pipeline_timer\(\)/);assert.match(build,/with trace:run\(/);
});
