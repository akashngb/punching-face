...
test('relay boots, upgrades a client, and echoes mock events', async () => {
  ...
  try {
    ...
    client.send(
      JSON.stringify({
        type: 'session.update',
        session: { instructions: 'test', tools: [{ name: 'react_to_hit' }] },
      }),
    );
    client.send(
      JSON.stringify({
        type: 'engine.event',
        event: { type: 'strike', region: 'cheek-left', force: 55 },
      }),
    );
    ...
    client.close();
    assert.ok(
      messages.some((m) => m.type === 'omni.session.ready'),
      'no ready event',
    );
    assert.ok(
      messages.some((m) => m.type === 'session.updated'),
      'no session.updated',
    );
    const call = messages.find(
      (m) => m.type === 'response.function_call_arguments.done',
    );
