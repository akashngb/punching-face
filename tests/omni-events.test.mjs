...
test('formatEvent produces one-line context for each type', () => {
  assert.equal(
    formatEvent({ type: 'press', region: 'cheek-right', pressure: 0.42 }),
    '[EVENT] press cheek-right pressure=0.42',
  );
  assert.equal(
    formatEvent({ type: 'release', region: 'cheek-right', speed: 0.71, rebound: true }),
    '[EVENT] release cheek-right speed=0.71 rebound=true',
  );
  assert.equal(
    formatEvent({ type: 'strike', region: 'cheek-left', force: 55 }),
    '[EVENT] strike cheek-left force=55',
  );
  assert.equal(
    formatEvent({ type: 'guard', state: true, region: 'head' }),
    '[EVENT] guard on region=head',
  );
  assert.equal(
    formatEvent({ type: 'consent', stage: 'scan', granted: true }),
    '[EVENT] consent scan granted',
  );
  assert.equal(
    formatEvent({ type: 'user.text', text: 'How is my guard?' }),
    '[USER] How is my guard?',
  );
});
