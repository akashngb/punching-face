...
class ReconstructionTests(unittest.TestCase):
    ...
    def test_saved_reconstruction_is_real_triangle_surface(self):
        ...
        self.assertGreater(float(np.quantile(area,.01)),1e-12)
        self.assertEqual(mesh['stats']['sourceSplats'],len(decode(Path('public/reference/face-surface-fixture.ply').read_bytes())[0]))

    def test_bad_splat_is_rejected(self):
        with self.assertRaises(ValueError):decode(b'not a splat','splat')
    ...
    def test_translucent_surface_samples_can_be_inspected_without_changing_source(self):
        ...
        dtype=np.dtype([('xyz','<f4',3),('scale','<f4',3),('rgba','u1',4),('rotation','u1',4)])
        points=np.zeros(100,dtype=dtype);points['xyz'][:,0]=np.linspace(-.1,.1,100);points['scale']=.003
        points['rgba']=[150,110,90,10];points['rotation']=[255,128,128,128];source=points.tobytes()
        with self.assertRaisesRegex(ValueError,'Too few valid Gaussian samples'):
            decode(source,'splat')
        self.assertEqual(len(decode(source,'splat',min_opacity=.01)[0]),100)
        self.assertEqual(source,points.tobytes())
        for invalid in [-.01,1,float('nan')]:
            with self.assertRaises(ValueError):decode(source,'splat',min_opacity=invalid)
