...
test('validate rejects missing required args', () => {
  const schema = {type: 'object', properties: {region: {type: 'string'}, pain: {type: 'number'}}, required: ['region']};
  assert.equal(validate(schema, {pain: 3})._error !== undefined, true);
});

test('validate rejects unknown enum values', () => {
  const schema = {type: 'object', properties: {phase: {type: 'string', enum: ['press', 'release']}}};
...
test('ToolRegistry dispatches to a handler and emits tool.applied', async () => {
  ...
  const result = await registry.dispatch({name: 'do_thing', arguments: {region: 'RLQ'}});
