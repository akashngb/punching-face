...
function installHair(spec){
  ...
  $('hair-description').textContent=spec?`Detected: ${spec.parameters.style||spec.parameters.type}. ${headHair.strandCount.toLocaleString()} ${spec.mode==='photo-detail'?'photo-guided detail strands':'estimated strands'}.`:'Rebuild a head scan with AI hair analysis to add editable strands.';
...
$('face-file').onchange=async e=>{
  ...
  try{
    if(file.size>120_000_000)throw new Error('Crop the face to a file smaller than 120 MB.');
    ...
    if(ext==='glb'){
      ...
      if(m.material.map&&m.userData.appearance){
        ...
        surfaceAppearance=new SurfaceAppearance(mesh.geometry,recovered.atlas,m.material.map.clone());headPivot.add(surfaceAppearance);g.dispose();setView('mesh');
...
async function restoreSession(data){
  ...
  if(data.appearanceAtlas&&mat.map){surfaceAppearance=new SurfaceAppearance(mesh.geometry,data.appearanceAtlas,mat.map.clone());headPivot.add(surfaceAppearance);setView('mesh');$('model-kind').textContent='Restored textured mesh · welded physics';}
...
async function loadPhotoFace(id){
  ...
  try{
    const asset=name=>`/api/face-asset?id=${id}&asset=${name}`;
    const [data,atlas,binding,cage]=await Promise.all(['mesh.json','texture-atlas.json','physics-binding.json','physics-cage.json'].map(name=>fetch(asset(name)).then(async r=>{if(!r.ok)throw new Error('Build the photo model to create '+name);return r.json();})));
    ...
    const texture=await new THREE.TextureLoader().loadAsync(asset('appearance.png'));texture.colorSpace=THREE.SRGBColorSpace;
    surfaceAppearance=new SurfaceAppearance(mesh.geometry,atlas,texture);headPivot.add(surfaceAppearance);capturedFaceId=id;sessionStorage.setItem('contact-active-capture',id);installGlasses(data.accessories?.glasses);installHair(data.accessories?.hair);
