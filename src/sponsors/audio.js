...
export function downsample(samples,fromRate,toRate){
  ...
  const ratio=fromRate/toRate,length=Math.floor(samples.length/ratio),out=new Float32Array(length);
...
export function base64ToBytes(text){const binary=atob(text),bytes=new Uint8Array(binary.length);for(let i=0;i<binary.length;i++)bytes[i]=binary.charCodeAt(i);return bytes;}
...
export function pcm16ToFloat32(bytes){
  const start=bytes.length>44&&bytes[0]===0x52&&bytes[1]===0x49&&bytes[2]===0x46&&bytes[3]===0x46?44:0;
  const count=(bytes.length-start)>>1,view=new DataView(bytes.buffer,bytes.byteOffset+start,count*2),out=new Float32Array(count);
  for(let i=0;i<count;i++)out[i]=view.getInt16(i*2,true)/0x8000;
  return out;
}

export const rms=samples=>{let sum=0;for(let i=0;i<samples.length;i++)sum+=samples[i]*samples[i];return Math.sqrt(sum/Math.max(1,samples.length));};
...
export class VoiceGate{
  constructor({startMs=120,endMs=700,minMs=350,maxMs=9000,calibrateMs=500,ratio=3.2,floor=.004}={}){Object.assign(this,{startMs,endMs,minMs,maxMs,calibrateMs,ratio,noise:floor,floor,speaking:false,loudMs:0,quietMs:0,spokenMs:0,heardMs:0,levels:[]});}
  // Returns 'start', 'end', 'discard' (too short to be speech) or null for each audio frame.
  push(level,frameMs){
    if(this.heardMs<this.calibrateMs){this.heardMs+=frameMs;this.noise=Math.max(this.floor,this.noise*.8+level*.2);return null;}
    ...
    if(!this.speaking){
      ...
      this.noise=Math.max(this.floor,this.noise+(level-this.noise)*(level<this.noise?.2:.002));
      this.loudMs=loud?this.loudMs+frameMs:0;
      if(this.loudMs>=this.startMs){this.speaking=true;this.quietMs=0;this.spokenMs=this.loudMs;this.levels=[];return 'start';}
      return null;
    }
    this.spokenMs+=frameMs;this.quietMs=loud?0:this.quietMs+frameMs;this.levels.push(level);
    ...
    if(this.quietMs>=this.endMs||timedOut){
      ...
      if(timedOut){const sorted=this.levels.slice().sort((a,b)=>a-b);this.noise=Math.max(this.noise,sorted[Math.floor(sorted.length*.2)]||0);}
      this.speaking=false;this.loudMs=0;this.quietMs=0;this.spokenMs=0;this.levels=[];
