...
export function zoneOf(point) {
  const [x, y] = point,
    side = x < 0 ? 'L' : 'R',
    centre = Math.abs(x) < 0.022;
  if (y > 0.045) return 'forehead';
  if (y > 0.015) return centre ? 'brow' : 'eye-' + side;
  if (y > -0.03) return centre ? 'nose' : 'cheek-' + side;
  if (y > -0.06) return Math.abs(x) < 0.03 ? 'mouth' : 'jaw-' + side;
  return centre ? 'chin' : 'jaw-' + side;
}

export class RoundStats {
  constructor({ windowMs = 30000, comboMs = 1500, comboSize = 3 } = {}) {
    Object.assign(this, {
      windowMs,
      comboMs,
      comboSize,
  ...
  add({ id = 'host', name = 'You', speed = 0, point = [0, 0, 0], side = null, time }) {
    const event = {
      ...
      speed,
      zone: zoneOf(point),
      side: side || (point[0] < 0 ? 'left' : 'right'),
      time,
    };
    const total = this.totals.get(id) || { name, count: 0, max: 0 };
    const triggers = [];
    if (speed > total.max + 0.15 && total.count >= 3) triggers.push('personal-best');
    total.name = name;
    total.count++;
    total.max = Math.max(total.max, speed);
    this.totals.set(id, total);
    this.events.push(event);
    this.trim(time);
    const recent = this.events.filter(
      (e) => e.id === id && time - e.time <= this.comboMs,
    );
  ...
  trim(now) {
    while (this.events.length && now - this.events[0].time > this.windowMs)
      this.events.shift();
  }
  ...
  snapshot(now, trigger) {
    ...
    for (const e of this.events) {
      ...
      p.count++;
      p.sum += e.speed;
      p.max = Math.max(p.max, e.speed);
      p[e.side]++;
      p.zones[e.zone] = (p.zones[e.zone] || 0) + 1;
      people.set(e.id, p);
    }
    const participants = [...people.values()]
      .map(({ sum, ...p }) => ({
        ...p,
        avg: +(sum / p.count).toFixed(2),
        max: +p.max.toFixed(2),
      }))
    ...
    return {
      ...
      last: this.last
        ? {
            name: this.last.name,
            zone: this.last.zone,
            speed: +this.last.speed.toFixed(2),
          }
        : null,
  ...
  scoreboard() {
    return [...this.totals.entries()]
      .map(([id, t]) => ({ id, name: t.name, count: t.count, max: +t.max.toFixed(1) }))
      .sort((a, b) => b.count - a.count);
  }
}
