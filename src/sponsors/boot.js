...
pill.onclick=()=>{dock.classList.toggle('collapsed');remember('open',dock.classList.contains('collapsed')?'0':'1');};
function show(tab){for(const b of dock.querySelectorAll('[data-tab]'))b.classList.toggle('active',b.dataset.tab===tab);for(const s of dock.querySelectorAll('[data-panel]'))s.classList.toggle('active',s.dataset.panel===tab);remember('tab',tab);}
...
async function start(){
  ...
  const arena=createArenaHost({...shared,panel:dock.querySelector('[data-panel=arena]'),coach,record});
  ...
  setInterval(()=>{
    const contact=window.__lastContact;if(!contact||contact.time===seen)return;
    record(contact,contact.source==='remote'?{id:'guest',name:'Guest'}:{id:'host',name:arena.hostName});
  },33);
  setInterval(()=>{dots.arena.className='sd-dot'+(arena.live?' live':config.livekit.configured?' on':'');dots.coach.className='sd-dot'+(config.omni.configured?' on':' warn');},1000);
