...
export const obs = {
  ...
  span(name, attributes, work) {
    return sdk
      ...
      : work({ setAttribute() {}, setStatus() {} });
  },
  log(message, attributes = {}) {
    sdk?.logger?.info(message, attributes);
  },
  warn(message, attributes = {}) {
    sdk?.logger?.warn(message, attributes);
  },
  error(error, context) {
    if (sdk) sdk.captureException(error, context ? { extra: context } : undefined);
