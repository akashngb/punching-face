...
const HAND_POSE_OPEN ={y:[-0.012, 0.022, 0.016,-0.004],z:[ 0.015, 0.004,-0.022,-0.026]};
const HAND_POSE_FIST ={y:[-0.008, 0.008,-0.008,-0.020],z:[ 0.020, 0.024, 0.012, 0.000]};
...
const NEON_HAND_MATERIAL=new THREE.LineBasicMaterial({color:0xb14dff,transparent:true,opacity:1,depthTest:false,depthWrite:false,blending:THREE.NormalBlending,toneMapped:false});
...
export class VirtualHand extends THREE.Group {
  ...
  demoPose(center,closed=0){
    ...
    for(let f=0;f<5;f++)for(let j=0;j<4;j++){
      ...
      points.push(new THREE.Vector3(x,center.y+yOff[j]-thumbDown,center.z+zOff[j]));
    }
    this.apply(points,t);
  }
}
...
export class Tracking {
  ...
  async start(){
    ...
    try{
      ...
      this.stream=await navigator.mediaDevices.getUserMedia({video:{width:{ideal:960},height:{ideal:540},frameRate:{ideal:60}},audio:false});
  ...
  scheduleFrame(){
    ...
    const post=async()=>{
      ...
      if(this.busy){this.scheduleFrame();return;}
      if(this.video.readyState<2){this.scheduleFrame();return;}
      this.busy=true;
      try{
        const posted=performance.now();
        const bitmap=await createImageBitmap(this.video);
        this.worker.postMessage({type:'frame',bitmap,timestamp:posted,capture:this.captureRequest,trackBody:this.armProfiles.size>0},[bitmap]);
        this.captureRequest=null;
      }catch{this.busy=false;this.scheduleFrame();}
    };
  ...
  tick(now,hands){
    ...
    for(const h of hands){h.tracked=false;h.armPose=null;}
...
export async function cropFacePortrait(canvas){
  ...
  const found=detector.detect(canvas);const lm=found.faceLandmarks?.[0];
  if(!lm)throw new Error('No face detected. Face the camera with even lighting and no obstructions.');
  ...
  const padSide=fw*0.55,padTop=fh*0.85,padBot=fh*0.35;
  ...
  if(cw<128||ch<128)throw new Error('Face is too small in the frame. Move closer to the camera and try again.');
...
export async function makePhotoFace(image){
  const detector=await ensureFaceDetector();
  const found=detector.detect(image);const lm=found.faceLandmarks?.[0];
