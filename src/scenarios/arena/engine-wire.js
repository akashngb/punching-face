...
import {
  play as playReaction,
  keyForEvent,
  warmUp,
  ARENA_KEYS,
} from '../../omni/reactions.js';
...
function readFlag() {
  ...
  try {
    return new URL(window.location.href).searchParams.get('arena_omni') === '1';
  } catch {
    return false;
  }
}
...
export function startArenaOmni({ stats, punchingFace = null, uiLog = null } = {}) {
  const lab =
    punchingFace || (typeof window !== 'undefined' ? window.__punchingFace : null);
  const bus = new EventBus();
  let session;
  try {
    session = new OmniSession();
  } catch {
  ...
  const avatar = {
    ...
    setExpression: (emotion, intensity) =>
      uiLog?.('event', `expression ${emotion} ${intensity}`),
  ...
  const registry = new ToolRegistry({
    bus,
    session,
    onError: (err) => uiLog?.('warn', err.message),
  });
  ...
  let mode = 'face';
  try {
    mode = localStorage.getItem('punching-face-sponsors-mode') || 'face';
  } catch {
    /* no localStorage */
  }
